...
class PoincareBallWrapped(PoincareBallParent):
    '''PoincareBall manifold for wrapped normal distribution.'''
    def __init__(self, dim=1, c=1.0):
        super().__init__(c)
...
class PoincareBall(Manifold):
    ...
    def sqdist(self, p1, p2, c):
        sqrt_c = c ** 0.5
        dist_c = artanh(
            sqrt_c * (self.mobius_add(-p1, p2, c, dim=-1) + self.min_norm).norm(dim=-1, p=2, keepdim=False)
        )
